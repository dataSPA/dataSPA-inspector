...
let DataSPAInspector = class DataSPAInspector extends LitElement {
    constructor() {
        ...
        this.maxPatchEvents = 20;
        this.maxSignalEvents = 20;
        this.signalTableNotObject = false;
        this.datastarFetchHandler = (evt) => {
            const { detail } = evt;
            if (!detail ||
                !['datastar-patch-elements', 'datastar-patch-signals'].includes(detail.type)) {
                return;
            }
    ...
    replayEvent(sseEvent) {
        return (_) => {
            document.dispatchEvent(new CustomEvent(DATASPA_FETCH_EVENT, {
                detail: { ...sseEvent, inspectorReplayed: true },
            }));
        };
    }
    sendToConsole(signal) {
        return (evt) => {
            evt.stopPropagation();
            evt.preventDefault();
    ...
    documentFragmentFromEvent(event) {
        if (!event) {
            return null;
        }
        if (!event.argsRaw) {
        ...
        const elementsWithSvgsRemoved = detailContent.replace(/<svg(\s[^>]*>|>)([\s\S]*?)<\/svg>/gim, '');
        const hasHtml = /<\/html>/.test(elementsWithSvgsRemoved);
        const hasHead = /<\/head>/.test(elementsWithSvgsRemoved);
        const hasBody = /<\/body>/.test(elementsWithSvgsRemoved);
        const newDocument = new DOMParser().parseFromString(hasHtml || hasHead || hasBody
            ? detailContent
            : `<body><template>${detailContent}</template></body>`, 'text/html');
    ...
    renderSSE() {
        return html `
      <div class="sse">
        ${this.elementPatchEvents.map((event) => html `
            <div class="event-row">
              <details>
    ...
    renderServerSignals() {
        return html `
      <div class="sse">
        ${this.signalPatchEvents.map((event) => html `
            <div class="event-row">
              <details>
...
${JSON.stringify(this.signals, null, 2)}
            ...
            : html `
              <table>
                <tbody>
                  ${map(Object.entries(this.signals), (key, _) => html `
                      <tr
    ...
    setMaxSignalEvents(event) {
        let newValue = parseInt(event.target.value);
        if (isNaN(newValue))
            return;
        if (newValue < this.maxSignalEvents &&
            this.signalPatchEvents.length > newValue) {
            this.signalPatchEvents = this.signalPatchEvents.slice(this.maxSignalEvents - newValue);
        }
        this.maxSignalEvents = newValue;
    }
    setMaxSSEEvents(event) {
        let newValue = parseInt(event.target.value);
        if (isNaN(newValue))
            return;
        if (newValue < this.maxPatchEvents &&
            this.elementPatchEvents.length > newValue) {
            this.elementPatchEvents = this.elementPatchEvents.slice(this.maxPatchEvents - newValue);
        }
...
__decorate([
    state()
], DataSPAInspector.prototype, "elementPatchEvents", void 0);
__decorate([
    state()
], DataSPAInspector.prototype, "signalPatchEvents", void 0);
__decorate([
    state()
], DataSPAInspector.prototype, "maxPatchEvents", void 0);
__decorate([
    state()
], DataSPAInspector.prototype, "maxSignalEvents", void 0);
__decorate([
    state()
], DataSPAInspector.prototype, "signalTableNotObject", void 0);
DataSPAInspector = __decorate([
    customElement('dataspa-inspector')
], DataSPAInspector);
export { DataSPAInspector };
//# sourceMappingURL=dataspa-inspector.js.map